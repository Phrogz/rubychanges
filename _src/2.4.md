...
* **Note:** Be aware that condition considered false if the _whole right hand_ of assignment is `false`/`nil`, not the first assigned variable:
  ...
  ```
* **Metadata:** `{kind:change, importance:medium, scope:(global)}`
...
* **Code:**
  ...
  ```
* **Metadata:** `{kind:change, importance:highlight, scope:(global)}`
...
* **Code:**
  ...
  # Ruby 2.4: => 100
  ```
* **Metadata:** `{kind:addition, importance:medium, scope:(global)}`
...
* **Code:**
  ...
  # => {1=>3, 3=>2, 2=>1}
  ```
* **Metadata:** `{kind:addition, importance:medium, scope:Module}`
...
* **Metadata:** `{kind:addition, importance:low, scope:Module}`


## Core

### `Warning` module added
...
* **Metadata:** `{kind:addition, importance:highlight, scope:Warning}`


### `Object#clone(freeze: false)` option
...
* **Metadata:** `{kind:addition, importance:low, scope:Object}`
...
* **Follow-up:** In Ruby 2.7, `clamp` also [allows](2.7.html#comparableclamp-with-range) passing range argument, which, especially when combined with [endless](2.6.html#endless-range-1) (2.6) and [beginless](2.7.html#beginless-range) (2.7) ranges, allows to use more powerful and idiomatic code:
  ...
  -123.clamp(0..)   # => 0
  ```
* **Metadata:** `{kind:addition, importance:highlight, scope:Comparable}`
...
* **Code:**
  ...
  # => Integer
  ```
* **Metadata:** `{kind:change, importance:highlight, scope:[Fixnum,Bignum,Integer]}`
...
* **Code:**
  ...
  ```
* **Metadata:** `{kind:addition, importance:low, scope:Integer}`
...
* **Code:**
  ...
  ```
* **Metadata:** `{kind:addition, importance:medium, scope:[Float,Integer,Rational]}`
...
* **Code:**
  ...
  ```
* **Metadata:** `{kind:addition, importance:highlight, scope:String}`
...
* **Note:** Be careful about subtle difference in encoding, when constructing an empty string:
  ...
  ```
* **Metadata:** `{kind:addition, importance:low, scope:String}`
...
* **Notes:** It was proposed (in the discussion above), but never implemented to allow passing options to `casecmp?`, making it more precise by specifying locales. Currently, some local characters can produce unexpected results:
  ...
  'ı'.upcase == 'I' # => true
  ```
* **Metadata:** `{kind:addition, importance:medium, scope:[String,Symbol]}`
...
#### `#match?` method for `String`, `Symbol`, and `Regexp`
...
* **Code:**
  ...
  ```
* **Metadata:** `{kind:addition, importance:medium, scope:[String,Symbol,Regexp]}`
...
* **Code:**
  ...
  ```
* **Metadata:** `{kind:addition, importance:medium, scope:MatchData}`
...
* **Code:**
  ...
  #  [false, ["e", "f"]], ...
  ```
* **Metadata:** `{kind:addition, importance:low, scope:Enumerable}`



#### `#sum`

`Enumerable#sum` was added as a core alternative of too common `reduce(:+)`. Also, provides more precise summation of floats by internally using Kahan's summation algorithm.
...
* **Note:** Separate implementation of `Array#sum` is provided for efficiency. Important thing to note is it doesn't rely on `Array#each` method:
  ...
  ```
* **Metadata:** `{kind:addition, importance:medium, scope:Enumerable}`
...
* **Metadata:** `{kind:addition, importance:low, scope:Array}`
...
* **Code:**
  ...
  a # => [1, 2, 3, 4, 5, 6]
  ```
* **Metadata:** `{kind:addition, importance:medium, scope:Array}`



#### `Array#pack` gains `buffer:` keyword argument
...
* **Note:** Notice the last example: when destructive version haven't changed a hash, it returns `nil` instead of hash itself. It is consistent with behavior of other destructive methods, and allows writing code like this:
  ...
  ```
* **Metadata:** `{kind:addition, importance:medium, scope:Hash}`
...
Affects `String#each_line`, `String#lines`, `IO#gets`, `IO#readline`, `IO#readlines`, `IO.foreach`, `IO.readlines`, `StringIO#gets`, `StringIO#each_line`, and `StringIO#readlines`.
...
* **Notes:** What is chomped (and what is lines split on) is controlled by `$/` global variable (dubbed `$RS` or `$INPUT_RECORD_SEPARATOR` by `English` module). While quite esoteric by today's standards, it could be really useful for one-off scripts that work with specific data:
  ...
  ```
* **Metadata:** `{kind:addition, importance:low, scope:[String,IO,StringIO]}`



#### `#empty?` method for filesystem objects

New method `empty?` was introduced into `Dir`, `File`, and `Pathname` classes to check if the file/directory is empty.
...
* **Code:**
  ...
  ```
* **Metadata:** `{kind:addition, importance:low, scope:[Dir,File,Pathname]}`
...
* **Metadata:** `{kind:addition, importance:low, scope:Thread}`
...
* **Code:**
  ...
  ```
* **Metadata:** `{kind:addition, importance:low, scope:TracePoint}`
