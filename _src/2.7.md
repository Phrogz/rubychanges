...
* **Status (as of <<date>>):** 2.7.6 is current _stable_
...
In addition to [endless range added in Ruby 2.6](2.6.html#endless-range-1) (`(start..)`), Ruby 2.7 introduces beginless ranges: `(..end)`.
...
#### Comments now allowed between chained method calls
...
#### Quotes (if present) in HERE-documents must be on the same line as document start
* **Code:**
  ```ruby
   <<"EOS
...
#### `yield` in singleton class syntax (was inconsistent with local variables accessibility) deprecated

* **Discussion:** [Feature #15575](https://bugs.ruby-lang.org/issues/15575).
* **Code:**
  ```ruby
  def foo
    x = 1
...
#### `$;` and `$,` global variables deprecated
These are the default `split` and `join` separators from Perl ancestry.
...
#### Implicit block capturing in `proc` and `lambda` deprecated
  ```ruby
  def foo
...
#### Flip-flop syntax deprecation from 2.6 **is reverted**

It turned out that for brevity in text-processing scrips (including one-liners run as `ruby -e "print <something>"`) the feature, however esoteric it may seem, has a justified usage.
...
### Refinements work with `#method`/`#instance_method`
...
#### `Module#const_source_location` added
...
#### `Module#autoload?` gains `inherit` argument
...
### `Comparable#clamp` accepts a `Range`
...
### `Complex#<=>` returns `nil` for imaginary
...
#### Certain core methods return frozen strings
...
* **Follow-up:** Instead of freezing `Symbol#to_s`, new method `Symbol#name` returning a frozen string [was introduced](3.0.html#symbolname) in 3.0.
* **Kind:** change
* **Importance:** low



#### `Symbol#start_with?` and `#end_with?` added
...
#### `Time#inspect` includes subseconds
