...
Ranges can be created without an end value
...
### Flip-flop syntax deprecated

Infamous esoteric flip-flop syntax is deprecated finally
* **Discussion:** [Feature #5400](https://bugs.ruby-lang.org/issues/5400)
...
#### `<Numeric>()` methods gain `exception:` argument

Use `exception: false` to return `nil` when conversion fails.
...
#### `#system` gains `exception:` argument
...
### `Module#method_defined?` gains `inherit` argument
...
### `String#split` with block yields each result
...
### `Proc` and `Method` can be composed
...
### `Array#union` and `Array#difference` added
...
### `Hash#merge` accepts multiple arguments
...
#### `#filter`/`#filter!` are aliases for `#select`/`#select!`
...
#### `#to_h` accepts a block to define mapping
...
#### `#step` returns `Enumerator::ArithmeticSequence`

`Range#step` and `Numeric#step` return an `Enumerator::ArithmeticSequence` instead of an instance of `Enumerator`.
...
#### `Dir#each_child` and `Dir#children` added
...
* **Discussion:** [Feature #15231](https://bugs.ruby-lang.org/issues/15231)
* **Note:** `NilClass#=~` is still allowed without deprecation notice.
* **Kind:** deprecation
* **Importance:** low
...
#### `Random.bytes` added
* **Discussion:** [Feature #4938](https://bugs.ruby-lang.org/issues/4938)
* **Doumentation:** [Random.bytes](https://ruby-doc.org/core-2.6/Random.html#method-c-bytes)
* **Code:**
  ```ruby
  Random.bytes(5)
  # => "8\a\xB0\xD1V"
...
* **Code:**
  ...
  ```
* **Follow-up:** In Ruby 2.7, `resolve_feature_path` was [moved](2.7.html#load_pathresolve_feature_path) to a `$LOAD_PATH` singleton method, and its behavior for already loaded paths was [fixed](2.7.html#resolve_feature_path-behavior-for-loaded-features-fixed) to return path nevertheless.
* **Kind:** addition
* **Importance:** medium
...
#### `TracePoint` adds new `:script_compiled` event
...
#### `TracePoint#enable` gains params `target:` and `target_line:`
